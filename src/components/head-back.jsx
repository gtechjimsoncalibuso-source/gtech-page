...
export default function Header() {
    ...
    // Determine if the bubble is in the bottom 40% of the screen
    ...
    useEffect(() => {
        const handleMove = (clientX, clientY) => {
            ...
            if (isMenuOpen) {
                if (isBottom) {
                    // If expanding UP, the bottom of the button can't go past screen height
                    // and the top of the menu can't go past 0
                    newY = Math.max(menuHeight, Math.min(newY, window.innerHeight - buttonHeight));
                } else {
                    // If expanding DOWN, the top of the button can't go past 0
                    // and the bottom of the menu can't go past screen height
                    newY = Math.max(0, Math.min(newY, window.innerHeight - menuHeight));
                }
            } else {
                // Standard boundaries when closed
    ...
    }, [isDragging, isMenuOpen, isBottom]); // Added isBottom to dependencies
    ...
    return (
        <header className="w-full h-[6rem] p-4 sticky top-0 flex flex-row items-center justify-between text-[#ffffffe5] bg-black z-[1000]">
            ...
            <nav 
                ...
                className={`fixed w-[3.5rem] h-[3.5rem] bg-[#f1f1f1] rounded-full shadow-md items-center justify-center z-[2000] transition-shadow ${isDragging ? 'shadow-2xl scale-105' : ''}`}
                style={{
                    left: `${position.x}px`,
                    top: `${position.y}px`,
                    touchAction: 'none'
            ...
            >
                {/* The Burger Button - This now stays exactly at position.x/y */}
                <button 
                ...
                {/* The Menu List - Positioned Absolutely so it doesn't push the button */}
                {isMenuOpen && (
                    <ul 
                        className={`absolute left-0 w-full p-[1rem] gap-4 flex flex-col items-center bg-[#f1f1f1] rounded-full shadow-lg border border-gray-300 
                        ${isBottom 
                            ? 'bottom-[4rem] flex-col' // Opens UPWARD (stays above the button)
                            : 'top-[4rem] flex-col'    // Opens DOWNWARD (stays below the button)
                        }`}
                    >
